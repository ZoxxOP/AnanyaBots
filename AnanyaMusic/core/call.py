...
from AnanyaMusic.utils.autoplay import fetch_autoplay_track, remember_played
...
from AnanyaMusic.utils.stream.queue import put_queue
from AnanyaMusic.utils.thumbnails import get_thumb as gen_thumb
...
class Call(PyTgCalls):
    ...
    async def autoplay_start(
        ...
        try:
            ...
            print(f"📥 AUTOPLAY: generating thumbnail for {vidid}")

            img = await gen_thumb(
                vidid,
                progress_percent=0,
                use_cache=False,
                user_name="🔁 ᴛᴇᴀᴍ-ᴀɴᴀɴʏᴀ-ʙᴏᴛꜱ",
            )

            print(f"✅ AUTOPLAY: thumbnail generated: {img}")
            ...
            print(f"✅ AUTOPLAY: thumbnail sent for {vidid}")

        except Exception as e:
            import traceback
            print(
                f"❌ AUTOPLAY THUMBNAIL/PHOTO ERROR "
                f"[{track.get('vidid')}]: {type(e).__name__}: {e}"
            )
            traceback.print_exc()
